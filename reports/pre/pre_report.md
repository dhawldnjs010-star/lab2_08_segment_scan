...
- 파형 캡처(VaporView): `evidence/pre/lab2_08_wave_full.png`(전체 Zoom Fit), `evidence/pre/lab2_08_wave_zoom.png`(상승 에지 확대)
...
|---|---|---|---|---|
| 정상 코드 | `<커밋/해시 기입>` | [normal.log](../../evidence/pre/lab2_08_normal.log) | 6 ns 기대 select=00, 실제 select=00. `LAB2_PASS segment_scan8 checks=194` | 모든 검사 통과, 1306 ns 종료. |
| 지정한 RTL 변경 | `<커밋/해시 기입>` | [mod.log](../../evidence/pre/lab2_08_mod.log) | 6 ns 기대 select=00, 실제 select=01. `LAB2_FAIL reset blanks digit zero time=6000`, `FATAL: sim/tb_segment_scan8.sv:12: check failed` | `reset blanks digit zero` 검사가 변경을 발견했다(로그의 time은 ps 단위, 6000 ps = 6 ns). |
| 원래 코드로 복구 | `<커밋/해시 기입>` | [recover.log](../../evidence/pre/lab2_08_recover.log) | 복구 후 전체 검사 재실행. `LAB2_PASS segment_scan8 checks=194`, `$finish called at 1306000 (1ps)` | PASS와 종료 시각이 정상 실행과 같고 새 VCD를 확인한다. |
